...
{
    public class ParticleSystemComponent : RendererComponent, IParticleSystemRenderData
    {
        ...
        public void Emit(int count = 1)
        {
            ...
            {
                Log.Default.Warn($"particel system has no particle properties or emitter assigned (entity id: {Entity.Id})");
                return;
            }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (Space == ParticleSpace.Global)
                    particle.position = Vector3.Transform(particle.position, Entity.Transform?.GetWorldTransformMatrix() ?? Matrix4x4.Identity);
        ...
        internal
#endif
        protected override void OnSerialize(SerializationInfo info)
        {
        ...
        internal
#endif
        protected override void OnDeserialize(SerializationInfo info)
        {
